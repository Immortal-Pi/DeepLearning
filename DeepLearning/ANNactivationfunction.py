...
def activation_function(activation_list):
    for i in range(len(activation_list)):
        model = Sequential([
            keras.layers.Flatten(input_shape=input_layer_size),
            keras.layers.Dense(hidden_layer_size, activation=activation_list[i]),
            keras.layers.Dense(output_layer_size, activation='softmax')
        ])
        model.compile(loss='categorical_crossentropy',optimizer=tf.optimizers.Adam(),metrics=['accuracy'])

        logdir=r'activation\\' +activation_list[i]
        cb=keras.callbacks.TensorBoard(log_dir=logdir,write_graph=False)
        print('building model '+activation_list[i]+' activation function')
        model.fit(x_train,y_train,batch_size=batch_size,epochs=epochs,callbacks=[cb],validation_split=0.2)
        print('model built successfully')

activation_list=['linear','sigmoid','tanh','relu','softmax']
