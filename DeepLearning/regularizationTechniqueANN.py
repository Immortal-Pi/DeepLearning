...
def regularization(list_of_regs,reg_names):
    ...
    for reg in range(len(list_of_regs)):
        model = Sequential([
            keras.layers.Flatten(input_shape=input_layer_size),
            keras.layers.Dense(hidden_layer_size, activation='relu',kernel_regularizer=list_of_regs[reg]),
            keras.layers.Dense(output_layer_size, activation='softmax')
            ])
        model.compile(loss='categorical_crossentropy', optimizer=tf.optimizers.Adam(), metrics=['accuracy'])
        logdir=r'regularization\\'+reg_names[reg]
        cb = keras.callbacks.TensorBoard(log_dir=logdir, write_graph=False)
        print('Building model '+reg_names[reg]+ ' Regularization')
